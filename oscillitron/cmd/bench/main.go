...
func run() error {
	...
	var (
		...
		copeConfSrc      = flag.String("cope-confidence-source", "semantic-entropy", "Thread B: which confidence column the cope dispatcher reads. 'semantic-entropy' = Answer.SEConfidence (Vote's discrete semantic entropy, 1−H/ln(N)) — the DEFAULT since H0-SE (2026-06-16) validated it: on GPQA Diamond / qwen3:14b it cut ECE 0.239→0.147 (delta +0.09, 3× the ≥0.03 build gate) and false-confident wrong ships 9→2 vs self-report. 'self' = Answer.Confidence (self-reported mean) restores pre-H0-SE behavior. See scratch/bench-results-2026-06-16.md + dense-router-design §2.12.3.")
