...
// fakeLMStudio is a minimal httptest server that mimics LMStudio's
...
func newFakeLMStudio() *fakeLMStudio {
	f := &fakeLMStudio{}
	f.server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(f.script) == 0 {
			f.mu.Unlock()
			http.Error(w, "fakeLMStudio: no scripted response left", http.StatusInternalServerError)
			return
		}
		next := f.script[0]
		f.script = f.script[1:]
...
func (f *fakeLMStudio) queue(r scriptedResponse) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.script = append(f.script, r)
}

func (f *fakeLMStudio) snapshotCalls() []capturedCall {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeLMStudio) close() { f.server.Close() }

func newAdapter(t *testing.T, url string, opts ...func(*Config)) *Adapter {
	t.Helper()
	cfg := SingleEndpoint(url, "qwen2.5-7b-instruct")
	for _, o := range opts {
		o(&cfg)
	}
	a, err := New(cfg)
...
func TestEvaluate_HappyPath(t *testing.T) {
	...
	if calls[0].model != "qwen2.5-7b-instruct" {
...
func TestName(t *testing.T) {
	...
	if a.Name() != "lmstudio-direct" {
		t.Errorf("Name = %q, want lmstudio-direct", a.Name())
	}
}
