...
//   - Confidence is also required even in minimal mode. The whole
//     v3 inadequacy-coping framework rests on confidence as the
//     primary "notice" signal — dropping the JSON envelope is fine,
//     but dropping the confidence number would throw the baby out
//     with the bathwater. We ask for it on a new line in a
//     parseable shape (`confidence: 0.X`) so a small follow-on
//     extractor can pull it from the raw text without re-imposing
...
//     bare text and wraps it into ReturnResultPayload. Confidence
//     extraction from the raw text is a separate concern (v3.2,
//     response-side signals).
const ProcessInstructions = `Answer the following multiple-choice question. Read it carefully and choose the best answer. End your response with exactly two lines:
1. The single letter (A, B, C, or D) of your final answer
2. confidence: X.X  (a number from 0.0 to 1.0 reflecting how sure you are)`
