...
// output protocol, not anything vLLM-specific. Consolidate into a
// shared package once a third substrate motivates it (this is the
...
func parseEvaluateResponse(raw string) (evaluatePayload, bool, error) {
	...
	if err := json.Unmarshal([]byte(obj), &p); err != nil {
		return evaluatePayload{}, false, fmt.Errorf("vllm: parse evaluate JSON: %w", err)
	}
	return p, true, nil
}
...
func parsePlaybook(s string) (session.Playbook, error) {
	...
	default:
		return "", fmt.Errorf("vllm: evaluate returned unknown playbook %q (expected one of plan|process|critique|verify_grounded|compose)", s)
	}
}
...
func parseRecomposeSpec(s string) (session.RecomposeSpec, error) {
	...
	default:
		return "", fmt.Errorf("vllm: plan returned unknown recompose spec %q", s)
	}
}
...
func parseVerdict(s string) (session.Verdict, error) {
	...
	default:
		return "", fmt.Errorf("vllm: critique/verify_grounded returned unknown verdict %q", s)
	}
}
...
func parseExecuteResponse(pb session.Playbook, raw string, require bool) (*session.Execute, error) {
	...
	if !ok {
		if require {
			return nil, fmt.Errorf("vllm: execute produced no structured envelope for %q (raw length %d)", pb, len(raw))
		}
		return unstructuredFallback(pb, raw), nil
	}
	switch pb {
...
func parseEmitSubtreeJSON(obj string) (*session.Execute, error) {
	...
	if err := json.Unmarshal([]byte(obj), &p); err != nil {
		return nil, fmt.Errorf("vllm: parse plan JSON: %w", err)
	}
...
func parseVerifierSignalJSON(obj string) (*session.Execute, error) {
	...
	if err := json.Unmarshal([]byte(obj), &p); err != nil {
		return nil, fmt.Errorf("vllm: parse verifier_signal JSON: %w", err)
	}
