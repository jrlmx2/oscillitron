...
type Config struct {
	...
	// for substrates that support it (Qwen3.x served by vLLM, etc.)
	// and silently ignores it on substrates that don't.
...
type chatRequest struct {
	...
	// Think — see pkg/adapter/ollama.chatRequest.Think. Pointer so
	// nil omits the field (substrate default) vs true/false explicit.
...
func (a *Adapter) oneCall(ctx context.Context, ep Endpoint, env session.Envelope, instructions, phase string, responseFormat map[string]any) (string, string, tokenUsage, string, error) {
	...
	if choice.Message.Reasoning != "" {
		trace.Info(a.cfg.Tracer, ctx, "vllm.thinking_emitted",
			slog.String("ap_id", string(env.ID)),
			slog.String("phase", phase),
			slog.String("model", ep.Model),
...
// maxTraceReasoningChars caps per-call reasoning trace size for
// trace events. See pkg/adapter/ollama for the rationale.
